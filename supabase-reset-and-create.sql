-- Reset and Create Complete Schema for newintegration.md
-- This script will drop existing tables and recreate everything from scratch

-- Drop all tables in correct order (reverse dependency order)
DROP TABLE IF EXISTS audit_logs CASCADE;
DROP TABLE IF EXISTS billing CASCADE;
DROP TABLE IF EXISTS overrides CASCADE;
DROP TABLE IF EXISTS synonym_events CASCADE;
DROP TABLE IF EXISTS synonyms_workspace CASCADE;
DROP TABLE IF EXISTS synonyms_global CASCADE;
DROP TABLE IF EXISTS results CASCADE;
DROP TABLE IF EXISTS extractions_norm CASCADE;
DROP TABLE IF EXISTS extractions_raw CASCADE;
...
DELETE FROM storage.objects WHERE bucket_id IN ('pdfs', 'exports');
DELETE FROM storage.buckets WHERE id IN ('pdfs', 'exports');
